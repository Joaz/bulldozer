...
#		res += cube([48,@args[:thickness],bearing_position-motor_position]).translate(z:motor_position)	
		...
		res += cube([48,47,motor_mount_thickness]).translate(z:motor_position).color(@@printed_color)	
		...
		[-1,1].each do |i|	
			[-1,1].each do |f|	
				...
				res += bolt.show.mirror(z:1).translate(x:26+i*15.7,y:25+f*15.7,z:motor_position+motor_mount_thickness) unless output == true
