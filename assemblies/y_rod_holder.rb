class YRodHolder < CrystalScad::Assembly
	...
	def part(show)
		res = cube([50,17+6,3]).center_xy.translate(y:-5) 
    ...
    res -= cylinder(d:12.7,h:20).translate(x:-8.5,z:2.5)
		...
		res += bolt.show.rotate(x:180).translate(x:15,y:-6.5,z:4) if show
		res += washer.show.rotate(x:180).translate(x:15,y:-6.5,z:4) if show
